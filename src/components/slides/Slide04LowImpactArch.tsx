export function Slide04LowImpactArch() {
  ...
  return (
    <div className="w-full max-w-6xl px-8">
      <div className="mb-8">
        <span className="font-mono text-xs text-accent-green">BLOQUE 03 // LOW_IMPACT_ARCH</span>
        <h2 className="font-sans text-5xl font-bold mt-2">Arquitecturas de Bajo Impacto</h2>
      </div>
        ...
        {cols.map(col => (
          <div key={col.label} className="bento-card p-10 flex flex-col">
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="font-mono text-xs text-accent-green">{col.label}</span>
            </div>
            <h3 className="font-sans text-3xl font-semibold mb-6">{col.title}</h3>
            <div className="space-y-4 flex-1">
              {col.items.map(item => (
                <div key={item.label} className="docker-container">
                  <div className="font-mono text-xs text-accent-green mb-1">{item.label}</div>
                  <div className="font-sans text-xs text-gray-400">{item.desc}</div>
                </div>
              ))}
            </div>
            <div className="mt-4 pt-4 border-t border-white/10">
              <span className="font-mono text-sm text-gray-500">{col.footer.split(':')[0] + ': '}</span>
              <span className="font-mono text-xs text-accent-green">{col.footer.split(':').slice(1).join(':')}</span>
            </div>
          </div>
        ))}
      </div>
